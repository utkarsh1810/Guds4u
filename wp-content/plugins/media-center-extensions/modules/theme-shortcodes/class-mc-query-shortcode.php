...
if ( ! function_exists( 'custom_wp_query' ) ) :
	function custom_wp_query($args = null, $content = null, $should_load_template = false ) {
		global $custom_query, $paged, $wp_query, $mc_plugin_dir;
		...
		if( ! empty( $wp_query_args[ 'layout' ] ) ) {
			$custom_query->layout = $wp_query_args[ 'layout' ];
		}
	    ...
	    if( $should_load_template ) {
	    	load_template( $mc_plugin_dir . 'templates/' . $wp_query_args[ 'template' ] );
	    } else {
	    	get_template_part( 'templates/layouts/' . $wp_query_args[ 'template'] );
	    }
